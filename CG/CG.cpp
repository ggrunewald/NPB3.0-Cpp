...
CG::CG(char c, int n, bool s) : CGBase(c, n, s)
{
    CG();
...
void CG::RunBenchMark()
{
    int i, j, k, it;
    double zeta;
    double rnorm = 0;

    cout << " Size: " << na << " Iterations: " << niter << endl;
...
//---------------------------------------------------------------------
    ...
    //TESTE
    //for(j=1;j<=naa+1;j++)
    //            cout << "z[" << j << "] = " << z[j] << " x[" << j << "] = "<< x[j] << endl;

    //cout <<"tnorm1 = " << tnorm1 << " tnorm2 = " << tnorm2 << endl;
...
void CG::Makea(int n, int nz, double * a, int * colidx, int * rowstr,
            ...
            if (jcol>=firstcol && jcol<=lastcol)
            {
                ...
                for(ivelt1=1;ivelt1<=nzv;ivelt1++)
                {
                    ...
                    if (irow>=firstrow && irow<=lastrow)
                    {
                        ...
                        if (nnza > nz)
                        {
                            cout << "Space for matrix elements exceeded in makea" << endl;
                            cout << "nnza, nzmax = " << nnza << ", " << nz << endl;
                            cout << " iouter = " << iouter << endl ;
                            return;
                        //System.exit(0);
                        }
...
double CG::Conj_grad( int * colidx, int * rowstr,
...
//---------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------
    //cout << "cgit = " << cgit << " cgitmax = " << cgitmax << endl;
...
//---------------------------------------------------------------------
    ...
    //TESTE
    //cout << "Teste . sqr(sum) = " << sum << endl;
    //cout << "Teste . sqrt(sum) = " << sqrt(sum) << endl;
    return sqrt( sum );
}

void CG::ExecuteTask(int OrderNum)
{

    for(int m=0;m<numThreads;m++)
    {
        worker[m]->SetTaskOrder(OrderNum);
        worker[m]->SetDone(false);
        worker[m]->SetAlpha(alpha);
        worker[m]->SetBeta(beta);
    }

    for(int m=0;m<numThreads;m++)
    {
        while(!worker[m]->IsDone());
    }
}

void CG::Sprnvc(int n, int nz, double * v, int * iv, int * nzloc,
...
int CG::Vecset(int n, double * v, int * iv,
...
void CG::Sparse(double * a, int * colidx, int * rowstr,
...
int CG::Verify(double zeta)
{
    	...
    	if(execClass != 'U')
    	{
      		...
      		{
    			verified = 1;
    			cout << " Deviation is   " << (zeta-zeta_verify_value)  << endl;
      		}
      		else
      		{
    	  		verified = 0;
    	  		cout << " The correct zeta is " << zeta_verify_value << endl;
      		}
    	}
    	else
    	{
...
double CG::GetMFLOPS(std::chrono::duration<double> total_time, int niter)
{
    double mflops = 0.0;
...
void CG::PrintTimers()
{
    ...
    std::chrono::duration<double> tempo = timer->ReadTimer(t_bench);

    double ttot = tempo.count();

    if (ttot == 0.0) ttot = 1.0;

    for(int i=1;i<=t_last;i++)
    {
        tempo = timer->ReadTimer(i);

        double tm = tempo.count();

        if(i==t_init)
        {
            cout << "  "<<t_names[i] << ":"<< tm << endl;
        }
        else
        {
            cout << "  "<<t_names[i] << ":"<< tm
...
void CG::SetupThreads(CG * cg)
{
    master = cg;

    int div = na/numThreads;

    int rem = na%numThreads;

    int start=1,end=0;

    dmaster = new double[numThreads];
    rhomaster = new double[numThreads];
    rnormmaster = new double[numThreads];
    ...
    for(int i=0;i<numThreads;i++)
    {
        ...
        worker.push_back(new CGWorker(cg,start,end));
